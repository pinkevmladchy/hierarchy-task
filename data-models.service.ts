...
export class DataModelAutoGeneratorService {
  ...
  private executeRequestsList(): Observable<any> {
    ...
    return forkJoin(entityObservables).pipe(
        ...
        switchMap(() => {
          return this.additionalElementsObservables.telemetries.length ? forkJoin(this.additionalElementsObservables.telemetries) : of(null);
        })
    );
  }
  ...
  private createDemoRelationsObservables() {
    console.log("SCHEMA TREE", this.schemaTree)
    this.latestCustomerId = null;
    const callBack = (treeEl, parent) => {
      ...
      parentEntities.forEach((parentEntity, idx) => {
        for (let i = 0; i < ratio; i++) {
          console.log("PERED ASSIGN", this.latestCustomerId)
          if(currentEntities[0].id.entityType === 'CUSTOMER'){
            debugger;
            this.latestCustomerId = currentEntities[0].id;
            console.log("PISLYA ASSIGN", this.latestCustomerId)
          }
          currentEntities[0].parent = parentEntity.id.id;
          if(currentEntities[0].id.entityType === 'ASSET' || currentEntities[0].id.entityType === 'DEVICE'){
            // console.log("YYYYYYYYYYYYYYYYYYYYYYY", this.latestCustomerId)
            this.entitiesForChangeOwners.push(this.entityGroupService.changeEntityOwner(this.latestCustomerId, currentEntities[0].id))
          }
